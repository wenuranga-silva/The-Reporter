    ...
    <div class="col-12">
        ...
        <div class="card shadow">
            ...
            <div class="card-header d-flex">
                ...
                @isset($news)
                    <h6 class="m-0 font-weight-bold text-primary align-content-center">Add Video - ({!! Str::substr($news->title, 0, 60) . '....' !!})</h6>
                @endisset

                @isset($video)
                    <h6 class="m-0 font-weight-bold text-primary align-content-center">Edit/Delete Video -
                        ({!! Str::substr($video[0]->News->title, 0, 60) . '....' !!})</h6>
                @endisset
            ...
            <div class="card-body">
                ...
                @isset($news)
                    ...
                    <form action="{{ route('admin.video.news.store') }}" method="POST" class="needs-validation" novalidate>
                        @csrf
                ...
                @isset($video)
                    ...
                    <form action="{{ route('admin.video.news.update') }}" method="POST" class="needs-validation" novalidate>
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-3 has-validation">
                            <label for="url" class="form-label">Url <code>*</code></label>
                            <input type="url" class="form-control {{ $errors->has('url') ? 'is-invalid' : '' }}"
                                id="url" name="url" value="{{ $video[0]->url }}">
                            <input type="hidden" name="post" value="{{ $video[0]->News->id }}">
                        ...
                        <div class="mb-3 has-validation">
                            ...
                            <textarea id="summernote" class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}"
                                name="description">{{ $video[0]->description }}</textarea>
                        </div>

                        <div class="mb-3 has-validation">
                            <label for="copyright_link" class="form-label">Video Copyright Link</label>
                            <input type="url" class="form-control {{ $errors->has('copyright_link') ? 'is-invalid' : '' }}"
                                id="copyright_link" name="copyright_link" value="{{ $video[0]->copyright_link }}">
                        </div>
                        ...
                        <div class="mb-3 has-validation">
                            ...
                            <input type="text"
                                ...
                                id="copyright_text" name="copyright_text" value="{{ $video[0]->copyright_text }}">
                        </div>
                        ...
                        <div class="mb-3 has-validation">
                            ...
                            <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status"
                                id="status">
                                <option value="yes" {{ $video[0]->status == 1 ? 'selected' : '' }}>Yes</option>
                                <option value="no" {{ $video[0]->status == 0 ? 'selected' : '' }}>No</option>
                            </select>
                        </div>
                        ...
                        <div class="mb-3 has-validation">
                            ...
                            <select id="category"
                                ...
                                @foreach ($categories as $category)
                                    <option value="{{ $category->id }}"
                                        {{ $video[0]->category_id == $category->id ? 'selected' : '' }}>
                                        {{ $category->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mt-4">
                            <button type="submit" class="btn btn-primary">Updte Video</button>
